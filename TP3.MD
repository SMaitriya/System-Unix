...
# Source du fichier d'existence d'utilisateur
fichier="liste_utilisateurs.sh"
source $fichier
...
read -p "Login : " login
check_login "$login"  # Vérifier si l'utilisateur existe déjà
read -p "Nom : " nom
read -p "Prénom : " prenom
read -p "UID : " uid
check_uid "$uid" # Vérifier si l'UID existe déjà
read -p "GID : " gid
